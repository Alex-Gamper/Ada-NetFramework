...
with Interfaces.C;
--------------------------------------------------------------------------------
package body NetFrameworkBase.System.Diagnostics.ProcessThreadCollection is
   ...
   This_TypeName     : constant Standard.Wide_String := "System.Diagnostics.ProcessThreadCollection";
   ...
   return NetFrameworkBase.System.Diagnostics.ProcessThread.Kind_Ptr is
      ...
      RetVal        : NetFrameworkBase.System.Diagnostics.ProcessThread.Kind_Ptr := new NetFrameworkBase.System.Diagnostics.ProcessThread.Kind;
   begin
   ...
   begin
      ...
      ------------------------------------------------------------
      p_Index(1) := 0;
      p_Value := GetObject (thread.m_Kind);
      Hr := SafeArrayPutElement (p_Parameters, p_Index(p_Index'first)'access, Convert (p_Value_Ptr));
   ...
   begin
      ...
      -- fixme parameter type := [array] System.Diagnostics.ProcessThread[]
   ...
   begin
      return RetVal : NetFrameworkBase.System.Diagnostics.ProcessThreadCollection.Kind_Ptr := new NetFrameworkBase.System.Diagnostics.ProcessThreadCollection.Kind do
      declare
         function Convert is new Ada.Unchecked_Conversion (LPVARIANT,LPVOID);
         Hr            : HResult := 0;
         p_Parameters  : aliased LPSAFEARRAY := null;
         ...
         p_Flags       : aliased NetFrameworkBase.UInt32 := 0;
         p0_Parameters : aliased LPSAFEARRAY := null;
         p0_Bounds     : aliased SAFEARRAYBOUND := (processThreads'Length , 0);
         p0_Index      : aliased array(1..1) of aliased LONG := (others => 0);
         p0_Tmp        : aliased IUnknown_Ptr;
         p0_Tmp_Ptr    : access IUnknown_Ptr := p0_Tmp'access;
      begin
         ...
         begin
            p0_Parameters := SafeArrayCreate (VT_UNKNOWN'enum_rep, 1, p0_Bounds'access);
            for i in processThreads'range loop
               p0_Index(1) := Interfaces.C.long(i) - 1;
               p0_Tmp := GetObject (processThreads(i).m_Kind);
               Hr := SafeArrayPutElement (p0_Parameters, p0_Index (p0_Index'first)'access, Convert (p0_Tmp));
            end loop;
            p_Value := To_Variant (p0_Parameters, VT_UNKNOWN);
         end;
